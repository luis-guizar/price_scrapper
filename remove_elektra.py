...
def main():
    ...
    try:
        print("Starting elektra cleanup...")
        ...
        prod_count = session.execute(text("SELECT count(*) FROM products WHERE source = 'elektra'")).scalar()
        hist_count = session.execute(text("SELECT count(*) FROM price_history WHERE product_id IN (SELECT id FROM products WHERE source = 'elektra')")).scalar()
        alert_count = session.execute(text("SELECT count(*) FROM alerts WHERE source = 'elektra'")).scalar()
        ...
        # 4. Delete in order with explicit commits
        print("Deleting alerts...")
        session.execute(text("DELETE FROM alerts WHERE source = 'elektra'"))
        session.commit()
        
        print("Deleting price history (this may take a moment)...")
        session.execute(text("DELETE FROM price_history WHERE product_id IN (SELECT id FROM products WHERE source = 'elektra')"))
        session.commit()
        
        print("Deleting products...")
        session.execute(text("DELETE FROM products WHERE source = 'elektra'"))
        session.commit()
