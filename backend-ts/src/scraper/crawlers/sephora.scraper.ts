...
export class SephoraScraper {
    ...
    async scrapeCategory(url: string): Promise<ScrapedProduct[]> {
        ...
        router.addDefaultHandler(async ({ request, log, body }: CheerioCrawlingContext) => {
            ...
            try {
                ...
                if (products.length > 0) {
                    log.info(`✅ Found ${products.length} products on page ${pageNumber}`);
                ...
                if (pageNumber === 1) {
                    ...
                    const pageSize = parseInt(new URL(currentUrl).searchParams.get('num_results_per_page') ?? '52');
        ...
        this.logger.log(`✅ Finished: Scraped ${totalScraped} products for "${term}".`);
        return allProducts;
    }
    ...
    private parseResult(result: any): ScrapedProduct | null {
        try {
            ...
            const image = imageRaw.startsWith('http') ? imageRaw : `${SEPHORA_BASE_URL}${imageRaw}`;
