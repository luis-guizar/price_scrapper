...
export class LiverpoolScraper {
    ...
    async scrapeCategory(url: string, progress?: ScrapeProgress): Promise<ScrapedProduct[]> {
        ...
        router.addDefaultHandler(async ({ request, log, json, body }: CheerioCrawlingContext) => {
            ...
            try {
                ...
                if (pageNumber === 1) {
                    ...
                    const totalNumRecords = data.mainContent?.totalNumRecords || data.mainContent?.contents?.[0]?.mainContent?.totalNumRecords;
        ...
        this.logger.log(`✅ Finished: Scraped ${totalScraped} products total for ${url}.`);
        return allProducts;
    }
    ...
    private parseSingleProduct(p: any): ScrapedProduct | null {
        try {
            ...
            const candidates = [promoPrice, salePrice, sortPrice, listPrice].filter(x => x > 0);
