...
export class MeliScraper {
  ...
  async scrapeCategory(
    ...
    const categoryLabel =
      url.split('/').pop()?.substring(0, 20) || 'meli-category';
    ...
    router.addDefaultHandler(
      async ({ page, request, log }: PlaywrightCrawlingContext) => {
        ...
        try {
          ...
          await page
            .waitForSelector('li.ui-search-layout__item', { timeout: 15000 })
            .catch(() => {
          ...
          if (products.length > 0) {
            ...
            if (currentDepth >= maxDepth) {
              log.info(
                `🛑 Reached max depth limit of ${maxDepth} pages for this category run.`,
              );
            } else if (totalScraped >= 1000) {
              log.info(
                `🛑 Reached max limit of 1000 items for this category run.`,
              );
            } else {
              ...
              if (hasNextPage) {
                ...
                log.info(
                  `🔗 Found next page, enqueuing (Depth: ${currentDepth + 1}): ${nextUrlStr}`,
                );
